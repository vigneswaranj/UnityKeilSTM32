...
#elif defined (WIN32)
#define __ASM             __asm                                      /*!< asm keyword for Visual Studio        */
  #define __INLINE         __inline                                  /*!< inline keyword for Visual Studio     */
#endif


/* ###################  Compiler specific Intrinsics  ########################### */
#if defined (WIN32)
#define __DSB()                                                  /*!< asm keyword for Visual Studio        */
...
/**
 * @brief  Outputs a character via the ITM channel 1
 *
 * @param   uint32_t character to output
 * @return  uint32_t input character
 *
 ...
 * It is blocking when a debugger is connected, but the previous character send is not transmitted. 
 */
static __INLINE uint32_t ITM_SendChChar (uint32_t ch, uint8_t chan)
{
  if(chan > 31)
  {
  	return 0;
  ...
  if ((CoreDebug->DEMCR & CoreDebug_DEMCR_TRCENA)  &&
      ...
      (ITM->TER & (1UL << chan))  ) 
  {
    while (ITM->PORT[chan].u32 == 0);
    ITM->PORT[chan].u8 = (uint8_t) ch;
  }  
  return (ch);
}
